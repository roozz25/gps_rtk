...
class TF_publisher :
    ...
    def point_callback(self,msg):
        ...
        # self.orientation.x = msg.pose.orientation.x
        # self.orientation.y = msg.pose.orientation.y
        # self.orientation.z = msg.pose.orientation.z
        # self.orientation.w = msg.pose.orientation.w
        self.orientation.x = 0
        self.orientation.y = 0
        self.orientation.z = 0
        self.orientation.w = 1
    ...
    def broadcast(self): 
        while not rospy.is_shutdown():
            ...
            # self.br.sendTransform(self.vector, self.orientation , self.time,"rover", "map")
            # self.rate.sleep()
            # #''current_time'' a la place de self.time dans la fonction
            current_time = rospy.Time.now()
            
            # Créer un message TransformStamped
            t = TransformStamped()
